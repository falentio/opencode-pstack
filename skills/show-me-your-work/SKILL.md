...
description: "Keep a reviewable decision trail for long-running or unattended work: a TSV log with one row per decision (what, why, evidence, result). Local by default; commit it when a reviewer needs the trail to trust the result. Load via the skill tool for show-me-your-work, autonomous or multi-phase runs, or work a human reviews after stepping away."
...
At the end of the run, before handing back, check the log told the truth. Read this run's session history via the opencode session API. Use only the current run's scope. Walk the log against what actually happened:
...
Before handing back, you must spawn an independent subagent on the inherited parent model. Self-review is not a substitute; the point is fresh eyes you cannot bring yourself. The subagent reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
...
Every reply for a run that produced a trail ends with an "Attention" section. Lead with `reviewed by an independent subagent` on its own line, then list each flag pointing to specific rows or moments. "No flags" is a valid value. The self-audit asks if the log told the truth; this asks what the user should still scrutinize even when it did.
