...
2. Pick the wake mechanism. There is no built-in loop command in opencode. An event to watch (CI, a merge, a ref advancing) gets a watcher subagent that wakes you on the event, with a long time-based heartbeat as fallback. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking. Repeat the predicate check in a background Task and poll on a heartbeat.
