---
name: automate-me
description: "Use for \"automate me\", \"create/update/refresh my -mode skill\", \"turn/capture my preferences or working style into a skill\", or wanting agents to follow how the user works. Drafts or revises a personal -mode skill via writing-great-skills and writing-for-agents + unslop, optionally pulling fresh evidence from recent transcripts."
...
Look recursively for `.agents/skills/**/*-mode/SKILL.md` and `~/.agents/skills/*-mode/SKILL.md` matching the user's handle. Mode skills can live in a personal category directory (`.agents/skills/<handle>/`), not only at the top level. If one exists, confirm intent with `question` (unless they already said "update my skill" or similar):
...
Use the `writing-great-skills` and `writing-for-agents` skills to author the skill. Placement:
...
Apply the **unslop** skill and the `writing-great-skills` and `writing-for-agents` writing guidelines to every line. Both apply to any agent-read prose, not just skills.
...
A `-mode` skill is subjective output. A formal test/iterate benchmark loop isn't useful here. Vibe-check with the user: does it read like them? Did it miss anything? Then ship.

Run a description-optimization loop only if the skill's trigger accuracy turns out to be a problem in practice.

## When not to use

- User wants a task-specific skill (not working conventions): `writing-great-skills` and `writing-for-agents` alone, no mining required.
...
- The `writing-great-skills` and `writing-for-agents` skills: skill authoring process and writing guidelines.
