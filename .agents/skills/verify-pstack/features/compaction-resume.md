...
- The unit suite imports from `src/`, not `dist/`. Pair it with the `plugin-skills` unit drive (which proves `dist/` registers the bundle) plus the `skill-live-load` drive (which proves `dist/` serves it) so the legs together cover source logic and shipped wiring.
