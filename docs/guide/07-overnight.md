...
loop until done via the autonomous-run playbook. if you're truly stuck after a few hours, stop and write up why.
```
...
- "loop until done" runs through the [Autonomous run playbook](../../skills/poteto-mode/playbooks/autonomous-run.md), which re-checks the finish condition on events or a heartbeat with a background Task and poll.
...
Before the skill hands back its summary, it spawns an independent reviewer on the inherited parent model to read the trail and the transcript, and the reply ends with an Attention section listing what deserves your scrutiny. Read that section first, then the log rows it points at. You're auditing decisions, not re-reading the whole night.
